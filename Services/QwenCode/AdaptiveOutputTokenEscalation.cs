...
/// <summary>
/// 对齐 Qwen Code「自适应输出 Token 扩容」设计（DeepSeek 网页 API 路径用启发式检测截断 + 多轮续写）。
/// 见 https://qwenlm.github.io/qwen-code-docs/zh/design/adaptive-output-token-escalation/
/// </summary>
public static class AdaptiveOutputTokenEscalation
{
        ...
        CancellationToken ct)
    {
        ...
        onLog?.Invoke("[Qwen Code] 检测到输出可能被截断，尝试续写…");
        ...
        for (var attempt = 1; attempt <= MaxRecoveryAttempts; attempt++)
        {
            ...
            onLog?.Invoke($"[Qwen Code] 续写第 {attempt}/{MaxRecoveryAttempts} 轮…");
        ...
        onLog?.Invoke("[Qwen Code] 续写次数已用尽，返回已生成部分。");
        return new WebChatResult
        {
