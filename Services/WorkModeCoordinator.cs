...
public sealed class WorkModeCoordinator
{
    ...
    public WorkModeStatePayload BuildState() =>
        WorkModeStatePayload.For(_mode, _webHost.IsAgentVisible);
    ...
    public Task BroadcastImmediateAsync(CancellationToken ct = default) =>
        _webHost.PushWorkModeStateNowAsync(BuildState(), ct);
    ...
    public string ToggleTargetMode() => _webHost.IsAgentVisible ? "chat" : "agent";
}
