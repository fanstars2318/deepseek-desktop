...
/// 桌面内各模块经 <see cref="IWebInjectBridge"/> 进程内直连，不经此服务。
/// </summary>
public sealed class LocalOpenAiServer : IDisposable
{
    ...
    private readonly WinUiWebInjectService _web;
    ...
    public LocalOpenAiServer(WinUiWebInjectService web) => _web = web;
    ...
    /// <summary>Agent 运行期间为 TUI 子进程提供临时 loopback 转发（非 5111；任务结束即停）。</summary>
    ...
    /// <summary>用户启用外部 OpenAI 兼容 API 时启动 HTTP 监听。</summary>
    ...
    private async Task<object> BuildHealthPayloadAsync()
    {
        var health = await _web.ProbeChat2ApiHealthAsync(_config.WebUserToken, BaseUrl)
                    ?? new Chat2ApiHealth { BaseUrl = BaseUrl, Error = "health probe unavailable" };
