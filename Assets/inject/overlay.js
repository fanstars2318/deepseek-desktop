(function () {
  ...
  const ICONS = {
    ...
    agent:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 2a4 4 0 0 1 4 4v1h1a3 3 0 0 1 3 3v9a3 3 0 0 1-3 3H7a3 3 0 0 1-3-3V10a3 3 0 0 1 3-3h1V6a4 4 0 0 1 4-4z"/></svg>',
  ...
  /** 挂在 html 上的 portal，避免 React 清空 body 时移除模式按钮 */
  function ensureModeFloaterHost() {
    let host = document.getElementById("ds-desktop-overlay-root");
  ...
  function attachFloaterToPortal(btn) {
    if (!btn) return;
  ...
  function bindWorkModeClient() {
    ...
    window.DsWorkMode.onChange(function (st) {
      ...
      if (isDeepSeekOfficialPage() && st.surface === "chat") {
        ensureStyles();
  ...
  function getFloaterLabel(btn) {
    return (
      btn.querySelector("#ds-agent-mode-float-label") ||
      btn.querySelector("#mode-float-label") ||
      btn.querySelector(".ds-native-label")
    );
  }
  ...
  function ensureAgentModeFloater() {
    ...
    btn.id = "ds-agent-mode-float";
    btn.type = "button";
    btn.className = "ds-mode-float ds-agent-mode-float ds-agent-mode-float-btn";
    btn.setAttribute("data-ds-floater", "1");
    btn.setAttribute("data-ds-workmode-floater", "1");

    const icon = document.createElement("span");
    icon.className = "ds-mode-float-icon";
  ...
  function mountAgentModeFloater() {
    try {
      ...
      if (isFloaterMounted()) {
        attachFloaterToPortal(document.getElementById("ds-agent-mode-float"));
        syncWorkModeUi();
        return;
      }
  ...
  window.__dsMountModeFloater = mountAgentModeFloater;
  ...
  function scheduleBurstInject(forceReset) {
    ...
    burstTimer = setTimeout(() => {
      ...
      const delays = hard ? [0, 200, 600] : [0, 400];
  ...
  function handleDesktopMessage(msg) {
    ...
    if (msg.type === "workModeState" && window.DsWorkMode) {
      ...
      if (shouldKeepNativeDeepSeekUi()) {
        ensureStyles();
        syncWorkModeUi(window.DsWorkMode.getState ? window.DsWorkMode.getState() : null);
      }
    }
  ...
  function onDomMutated() {
    if (shouldShowModeFloater() && !isFloaterMounted()) mountAgentModeFloater();
    if (mutTimer) return;
    mutTimer = setTimeout(() => {
      mutTimer = null;
        ...
        window.__dsFloaterInterval = setInterval(() => {
          if (shouldShowModeFloater()) {
            if (!isFloaterMounted()) mountAgentModeFloater();
              ...
              const b = document.getElementById("ds-agent-mode-float");
              if (b) attachFloaterToPortal(b);
            }
          }
          if (!shouldKeepNativeDeepSeekUi() && !isToolbarHealthy()) tryInject();
          updateApiDot();
