...
/// <summary>DSD API 多轮会话：client session_id → DeepSeek 网页 chat_session_id。</summary>
public sealed class DsdApiSessionStore
{
    ...
    public IReadOnlyList<DsdApiSessionInfo> ListSessions(AppConfig config)
    {
        lock (_lock)
        {
            PurgeExpiredLocked(config);
            return _sessions.Select(kv => new DsdApiSessionInfo
            {
    ...
    {
        var timeoutMs = Math.Max(1, config.DsdApiSessionTimeoutMinutes) * 60_000L;
    ...
    {
        var max = Math.Max(1, config.DsdApiMaxMessagesPerSession);
