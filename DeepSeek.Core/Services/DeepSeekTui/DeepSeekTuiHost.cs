...
public sealed class DeepSeekTuiHost : IAsyncDisposable
{
    ...
    private string? _lastAppliedLlmBaseUrl;
    ...
    public async Task EnsureRunningAsync(AppConfig config, CancellationToken ct)
    {
        ...
        var currentLlmBase = InternalChatChannel.ResolveTuiLlmBaseUrl(config) ?? "";

        if (_process is { HasExited: false } &&
            string.Equals(_lastAppliedLlmBaseUrl, currentLlmBase, StringComparison.Ordinal) &&
        ...
        {
            _process = proc;
            _lastAppliedLlmBaseUrl = currentLlmBase;
        }
    ...
    public static void ApplyProviderEnv(ProcessStartInfo psi, AppConfig config, string? runtimeBearerToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(tuiBaseUrl))
            psi.Environment["DEEPSEEK_BASE_URL"] = tuiBaseUrl;
        else
            psi.Environment.Remove("DEEPSEEK_BASE_URL");

        psi.Environment["DEEPSEEK_API_KEY"] = !string.IsNullOrWhiteSpace(config.DeepSeekApiKey)
            ? config.DeepSeekApiKey.Trim()
            : (!string.IsNullOrWhiteSpace(config.WebUserToken)
                ? config.WebUserToken.Trim()
                : DeepSeekDesktopApp.LocalApiKeyFallback);
        ...
        var dispatcher = DeepSeekTuiBundle.ResolveDispatcher(config.DeepSeekTuiExecutablePath, config);
        if (!string.IsNullOrWhiteSpace(dispatcher))
        {
            var toolsDir = DeepSeekTuiBundle.ResolveCompanionDirectory(dispatcher, config)
                           ?? DeepSeekTuiBundle.ToolsDirectory;
            var runtimePath = Path.Combine(toolsDir, "deepseek-tui.exe");
            if (File.Exists(runtimePath))
                psi.Environment["DEEPSEEK_TUI_BIN"] = Path.GetFullPath(runtimePath);
        }
    }
